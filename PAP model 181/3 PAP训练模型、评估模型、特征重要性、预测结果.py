...
df = pd.read_csv("F:/8PX_train_14_descriptor_RFE622.csv",index_col = 0)   # index_col=0——第一列为索引值
...
plt.savefig('F:/文章一 活性预测/Figure/fig 1 流程图/622 px ROC曲线.pdf', dpi=300)   # 保存图片到F盘，清晰度dpi
plt.show()
...
#plt.savefig('F:/文章一 活性预测/Figure/fig S1-15/px description importance622.pdf', dpi=300)   # 保存图片到F盘，清晰度dpi
plt.show()
...
df = pd.read_csv("F:/8new px 14 descriptor622.csv",index_col = 0)   # index_col=0——第一列为索引值
